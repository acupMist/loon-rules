...
const SALT_WEB = "d9200c846b10886e8c874fc33c8f308b"; // client_type 5
const SALT_APP = "47f15f1b66bee46b816115d8e8e6ebb6"; // client_type 1
const CLIENT_WEB = "5";
const CLIENT_APP = "1";
const APP_ID = "bll8iq97cem8";
const LOG_TAG = "[genshin-sign]";
...
var NOTIFY_ENABLED = parseArgFlag(RAW_ARGUMENT, "notify", true);

function postNotify(title, subtitle, body) {
  var t = title || "";
  var sub = subtitle || "";
  var b = body || "";
  log("notify →", t, "|", sub, "|", b);
...
function notify(title, subtitle, body) {
  if (!NOTIFY_ENABLED) {
    log("notify gated OFF, skip:", title, subtitle);
    return;
  }
  postNotify(title, subtitle, body);
}

/** 启动通知：不受 notify 开关控制，便于诊断 */
function notifyBoot(title, subtitle, body) {
  postNotify(title, subtitle, body);
}
...
function load(k) {
  try {
    ...
    log("load error", k, e && e.message ? e.message : e);
    return "";
  }
}
function save(k, v) {
...
function httpReq(opt) {
  return new Promise(function (resolve, reject) {
    ...
    log("http", method, opt.url);
    var cb = function (err, resp, body) {
      if (err) {
        log("http err", opt.url, err);
        reject(err);
      } else {
        log("http ok", opt.url, "status=", resp && resp.status, "len=", body != null ? String(body).length : 0);
        resolve({ resp: resp, body: body, status: resp && resp.status });
      }
    };
...
function ensureDevice() {
  ...
  if (!id) { id = uuid(); save(KEY.deviceId, id); log("generated deviceId"); }
  var fp = load(KEY.deviceFp);
  if (!fp) { fp = randStr(13); save(KEY.deviceFp, fp); log("generated deviceFp"); }
  return { id: id, fp: fp };
}
...
async function fetchRoles(cookie, device) {
  ...
  var role =
    ...
    list[0];
  log("fetchRoles ok uid=", role.game_uid, "region=", role.region);
...
async function fetchInfo(cookie, device, uid, region) {
  log("fetchInfo uid=", uid, "region=", region);
  var url =
    SIGN_BASE +
...
async function postSign(cookie, device, uid, region) {
  log("postSign uid=", uid, "region=", region);
  var r = await httpReq({
...
async function refreshTokens(cfg, device) {
  log("refreshTokens start, hasStoken=", !!cfg.stoken, "hasMid=", !!cfg.mid);
  ...
  try {
    ...
    if (vj.retcode === 0 && vj.data && vj.data.new_token && vj.data.new_token.token) {
      ...
      cfg.stoken = stoken;
      log("verify refreshed stoken");
    }
  } catch (e) {
    log("verify error", e && e.message ? e.message : e);
  }
  ...
  if (ej.retcode !== 0 || !ej.data || !ej.data.token || !ej.data.token.token) {
    log("exchange fail", ej.message || er.body);
  ...
  log("exchange ok, cookie updated");
  return { ok: true };
}
...
async function main() {
  ...
  log("cookie present=", !!cookie, "len=", cookie ? cookie.length : 0);
  if (!cookie) {
  ...
  log("device id=", device.id, "fp=", device.fp);
  ...
  log("cfg uid=", cfg.uid, "region=", cfg.region, "hasStoken=", !!cfg.stoken, "mid=", !!cfg.mid, "stuid=", !!cfg.stuid);
  ...
  try {
    ...
    log("info retcode=", info.retcode, "msg=", info.message, "is_sign=", info.data && info.data.is_sign);
    if (isAuthErr(info.retcode, info.message) && cfg.stoken) {
      ...
      log("info after refresh retcode=", info.retcode);
    }
    if (info.retcode !== 0) {
    ...
    log("sign retcode=", result.retcode, "msg=", result.message);
    if (isAuthErr(result.retcode, result.message) && cfg.stoken) {
      var ref2 = await refreshTokens(cfg, device);
      if (ref2.ok) {
        result = await postSign(cfg.cookie, device, cfg.uid, cfg.region);
        log("sign after refresh retcode=", result.retcode);
      }
    }
    ...
    if (result.retcode === 0) {
      ...
      try {
        ...
        log("post-sign info error", e && e.message ? e.message : e);
      }
    ...
    log("main catch", e && e.message ? e.message : e);
    notify("原神签到", "脚本异常", String(e && e.message ? e.message : e));
  }
}

// —— 同步启动诊断（在任何 async 之前）——
log("boot sync: script loaded");
...
if (typeof RAW_ARGUMENT === "object" && RAW_ARGUMENT && "notify" in RAW_ARGUMENT) log("$argument.notify =", RAW_ARGUMENT.notify);
log("NOTIFY_ENABLED =", NOTIFY_ENABLED);
log("has $notification.post =", !!(typeof $notification !== "undefined" && $notification && typeof $notification.post === "function"));
log("has $notify =", typeof $notify === "function");
log("has $httpClient =", typeof $httpClient !== "undefined");
log("has $persistentStore =", typeof $persistentStore !== "undefined");
log("has $done =", typeof $done === "function");
...
notifyBoot(
  ...
  "notify=" +
    ...
    "\n$argument=" +
    String(RAW_ARGUMENT) +
    "\n请查看 Loon→脚本→日志 中的 [genshin-sign] 行"
);

// Keep script alive until async finishes so Loon does not exit before notify; $done once
Promise.resolve()
  .then(function () {
    log("Promise chain start → main()");
    return main();
  })
  .catch(function (e) {
    log("Promise catch", e && e.message ? e.message : e);
    notify("原神签到", "脚本异常", String(e && e.message ? e.message : e));
  })
  .then(
    function () {
