/**
 * 原神 · 米游社每日签到（国服 luna/hk4e）— Loon 脚本
 *
 * 抓包还原：
...
function tryCapture() {
  ...
  var url = $request.url;
  if (
    !/api-takumi\.mihoyo\.com\/event\/luna\/hk4e\//.test(url) &&
    !/passport-api\.mihoyo\.com\/account\/ma-cn-session/.test(url) &&
    !/act\.mihoyo\.com\/bbs\/event\/signin/.test(url)
  ) {
    return false;
  }
  ...
  var cookie = h["cookie"] || "";
  if (cookie && /cookie_token/i.test(cookie)) {
  ...
  if (h["x-rpc-device_id"]) save(KEY.deviceId, h["x-rpc-device_id"]);
  if (h["x-rpc-device_fp"]) save(KEY.deviceFp, h["x-rpc-device_fp"]);
  notify("原神签到", "捕获成功", "已保存 Cookie / 设备信息（请勿外传）");
  return true;
}
...
async function main() {
  ...
  if (!cookie) {
    notify("原神签到", "未配置 Cookie", "请写入 genshin_cookie，或用 http-request 捕获。见 README。");
    $done();
    return;
  }
